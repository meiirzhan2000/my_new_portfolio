...
<template>
    ...
    >
      <div class="container-custom py-4">
        <nav class="flex items-center justify-between">
          <!-- Animated Logo -->
          <NuxtLink to="/" class="text-xl font-bold text-primary-400 logo-animation">
            {{ $t('nav.portfolio') }}
          </NuxtLink>
          
          <!-- Navigation Links with Staggered Animation -->
          <ul class="hidden md:flex items-center space-x-8 nav-items-container" :class="{ 'is-visible': isHeaderVisible }">
            <li 
  ...
  >
    {{ $t(`nav.${item.key}`) }}
    <span class="absolute -bottom-1 left-0 w-0 h-0.5 bg-primary-400 transition-all group-hover:w-full"></span>
  </NuxtLink>
            </li>
            ...
            <li class="nav-item" :style="{ 'animation-delay': '0.9s' }">
              ...
              >
                <span class="text-sm font-medium">{{ isEnglish ? 'RU' : 'EN' }}</span>
              </button>
            </li>
          </ul>
      ...
      <div class="container-custom py-8">
        <div class="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
          ...
          <div class="flex space-x-4">
            ...
            >
              <fa-icon :icon="link.icon" class="text-xl text-neutral-400 hover:text-primary-400 transition-colors" />
            </a>
          </div>
        </div>
      </div>
...
// Placeholder social links (these would come from Supabase in a real implementation)
